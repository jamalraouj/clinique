...
class ChambreController extends AbstractController
{
    ...
    public function new(Request $request, ChambreRepository $chambreRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // ->add('creer_a')
            // ->add('mise_a_jour_a')
            $chambreRepository->add($chambre->setCreerA(\DateTime::createFromFormat('Y-m-d H:i:s', date('Y-m-d H:i:s'))));
            $chambreRepository->add($chambre->setMiseAJourA(\DateTime::createFromFormat('Y-m-d H:i:s', date('Y-m-d H:i:s'))));
